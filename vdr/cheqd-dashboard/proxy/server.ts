...
const PORT: number = parseInt(process.env.PROXY_PORT || '3031');
const CHEQD_RPC_URL: string | undefined = process.env.NEXT_PUBLIC_CHEQD_RPC_URL;
const FRONTEND_URL: string = `http://localhost:${process.env.PORT}`;
const REQUEST_TIMEOUT: number = parseInt(process.env.REQUEST_TIMEOUT || '15000');
...
const handleProxyError = (error: AxiosError, req: Request, res: Response, serviceType: string): void => {
...
// RPC proxy
app.use('/api/rpc', async (req: Request, res: Response) => {
  try {
    ...
    const config: AxiosRequestConfig = {
      method: req.method.toLowerCase() as 'get' | 'post' | 'put' | 'delete' | 'patch' | 'options' | 'head',
      url: url,
      timeout: REQUEST_TIMEOUT,
      headers: {
    ...
    handleProxyError(error as AxiosError, req, res, 'RPC');
  }
});
